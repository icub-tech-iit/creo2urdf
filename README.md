...
- To export the urdf of the robot, the CREO mechanism has to follow [these guidelines](https://github.com/mesh-iit/cad-libraries/wiki/Prepare-PTC-Creo-Mechanism-for-URDF).
- The joints are assumed to be in position 0.
- Right now it can handle only **REVOLUTE**, **PRISMATIC**, **FIXED** and **BALL** joints.

> [!note]
> The **BALL** joints are not supported in the URDF, for this reason they are exported as a chain of three **REVOLUTE** joints, with axis orthogonal each other, intersecting in one point, and with the two middle links of the chain with mass zero.
...
Follow the [`Usage`](https://github.com/mesh-iit/creo2urdf/tree/master#usage) section instructions for completing the installation. <br>
...
>[!note]
>`creo2urdf` uses a [`vcpkg.json`](https://github.com/mesh-iit/creo2urdf/blob/master/vcpkg.json#L15) for installing the specific version of the dependencies needed for the compilation.
>The version of vcpkg used is specified [here](https://github.com/mesh-iit/creo2urdf/blob/3ff282240344ff2703e0130023eb8a11d50ef5f9/vcpkg.json#L15).

## Usage

- Put in your CREO working directory the `protk.dat` that is automatically generated by CMake in `${PROJECT_BINARY_DIR}` (e.g. `C:\Users\ngenesio\mesh-iit\creo2urdf\build\x64-Release`).
