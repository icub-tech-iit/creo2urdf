...
| `exportFirstBaseLinkAdditionalFrameAsFakeURDFBase` | Boolean | false | Set the value of the `iDynTree::ModelExporterOptions::exportFirstBaseLinkAdditionalFrameAsFakeURDFBase` option to the underlying iDynTree URDF exporter. To avoid confusion (see https://github.com/icub-tech-iit/creo2urdf/issues/140 and https://github.com/robotology/idyntree/issues/1201) the default value of this parameter when set is false, but setting it explicitly to `true` can be used to obtain the default behaviour in creo2urdf versions up to v0.5.11 . | 
